...
def _guide(
    feature: dict[str, str], *, label: str, label_y: float, line_top: float, baseline: float, css_class: str
) -> str:
    x = spectrum.wavelength_x(float(feature["wavelength_nm"]))
    return (
        f'<line x1="{x:.2f}" y1="{line_top}" x2="{x:.2f}" y2="{baseline}" class="{css_class}" />'
        f'<text x="{x:.2f}" y="{label_y}" class="guide-label">{escape(label)}</text>'
    )
...
def render_line_spectrum_svg(selected_feature_label: str) -> str:
    ...
    top, baseline = 82.0, 190.0
    lines = "".join(
        f'<line x1="{spectrum.wavelength_x(float(feature["wavelength_nm"])):.2f}" y1="{top}" '
...
.axis,.tick {{ stroke:#64748b; stroke-width:1.5; }} .tick-label {{ fill:#334155; font-size:18px; text-anchor:middle; }}
...
.guide-label {{ fill:#17212b; font-size:18px; font-weight:650; text-anchor:middle; }} .axis-label {{ fill:#17212b; font-size:20px; font-weight:650; text-anchor:middle; }}
</style><svg class="read-line-svg" viewBox="0 0 1200 250" role="img" aria-label="Six selected hydrogen lines on a wavelength axis">
...
<line x1="{spectrum.PLOT_LEFT}" y1="{baseline}" x2="{spectrum.PLOT_RIGHT}" y2="{baseline}" class="axis" />{_ticks(baseline)}{lines}
{_guide(example, label="Example line", label_y=30, line_top=56, baseline=baseline, css_class="trace-example")}
{_guide(selected, label="Your trace", label_y=56, line_top=68, baseline=baseline, css_class="trace-selected")}
<text x="{(spectrum.PLOT_LEFT + spectrum.PLOT_RIGHT) / 2}" y="242" class="axis-label">Wavelength / nm</text></svg>'''
...
def render_intensity_graph_svg(selected_feature_label: str) -> str:
    ...
    example = feature_for_label(EXAMPLE_FEATURE_LABEL)
    selected = feature_for_label(selected_feature_label)
    baseline, peak = 190.0, 190.0 - ILLUSTRATIVE_PEAK_HEIGHT
    peaks = "".join(
...
.trace-selected {{ stroke:#8A68C8; stroke-width:3; stroke-dasharray:4 4; vector-effect:non-scaling-stroke; }}
.guide-label {{ fill:#17212b; font-size:18px; font-weight:650; text-anchor:middle; }} .axis-label {{ fill:#17212b; font-size:20px; font-weight:650; text-anchor:middle; }}
</style><svg class="intensity-graph-svg" viewBox="0 0 1200 250" role="img" aria-label="Illustrative equal-height hydrogen intensity peaks on a wavelength axis">
