...
**DECISION:** Explore spectra is a direct, reversible data-exploration surface. Learners independently choose emission and/or absorption, the neutral atoms to inspect, and one or more visual spectrum, wavelength spectrum, or intensity-versus-wavelength representations. They can arrange the evidence primarily by representation or atom without changing the selected data. Emission and the visual spectrum start selected, and all five atoms start in view.
...
Whiteboard and paper remain the thinking surfaces for reasoning about energy levels, reading gaps, and calculating a hydrogen wavelength. The app does not reproduce that work with an energy-level diagram or calculator. **DECISION:** Hydrogen uses a prediction-first barcode → adaptive quantitative comparison. A valid supported transition and plausible submitted prediction replace the barcode with stored Balmer evidence in a local comparison; the learner-entered value remains unchanged, evidence persists for the session, and transition labels appear only after the reveal. **DECISION:** Nearest-nm precision is sufficient for the CHEM 1A calculation and comparison; learner-facing observed values use that precision while full NIST precision remains in the scientific data and provenance layer. **DECISION:** The adaptive comparison window has a minimum 20 nm span and widens to contain both the prediction and intended observed line. **DECISION:** The app may flag a prediction near another stored Balmer line, but does not infer detailed algebra misconceptions; deeper diagnosis remains with the learner and tutor.
